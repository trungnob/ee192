...
void My_IRQ_Function() {				// Interupt service Routine
	...
	{	
		...
		if(Mode == RACING) {
			GP2DAT &= ~(0x10100000); //turn off lights to indicate the Constant mode
			...
			//GP2SET = 0xFF20FFFF;
			GP2DAT |= 0x10100000; //Configure the default Learning mode and also set the 3 pins to be output
		}
			...
			//GP2SET = 0xFF10FFFF;
			GP2DAT |= 0x10100000; //All lights on means Racing mode
